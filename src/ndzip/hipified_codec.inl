...
#include <ndzip/hipified.hh>
#include <ndzip/offload.hh>


namespace ndzip::detail::gpu_hipified {
...
template<typename Profile>
class hipified_compressor_impl final : public hipified_compressor<typename Profile::value_type> {
  public:
    using value_type = typename Profile::value_type;
    using bits_type = typename Profile::bits_type;

    explicit hipified_compressor_impl(hipifiedStream_t stream, const compressor_requirements &reqs);
  ...
  private:
    ...
    template<typename>
    friend class hipified_offloader;

    hipified_buffer<bits_type> chunks_buf;
    hipified_buffer<index_type> chunk_lengths_buf;
    std::vector<detail::gpu_hipified::hipified_buffer<index_type>> intermediate_bufs;
    hipifiedStream_t _stream;
};


template<typename Profile>
hipified_compressor_impl<Profile>::hipified_compressor_impl(hipifiedStream_t stream, const compressor_requirements &reqs)
    : _stream{stream} {
    ...
    chunk_lengths_buf.allocate(detail::ceil(1 + num_chunks, detail::gpu_hipified::hierarchical_inclusive_scan_granularity));
    intermediate_bufs = detail::gpu_hipified::hierarchical_inclusive_scan_allocate<index_type>(chunk_lengths_buf.size());
}

template<typename Profile>
void hipified_compressor_impl<Profile>::compress(const value_type *in_device_data, const extent &data_size,
...
template<typename Profile>
class hipified_decompressor_impl final : public hipified_decompressor<typename Profile::value_type> {
  public:
    using value_type = typename Profile::value_type;
    using bits_type = typename Profile::bits_type;

    hipified_decompressor_impl() = default;

    explicit hipified_decompressor_impl(hipifiedStream_t stream) : _stream(stream) {}
  ...
  private:
    ...
    hipifiedStream_t _stream = nullptr;
};

template<typename Profile>
void hipified_decompressor_impl<Profile>::decompress(
...
template<typename Profile>
class hipified_offloader final : public offloader<typename Profile::value_type> {
  public:
    using value_type = typename Profile::value_type;
    using bits_type = typename Profile::bits_type;
...
template<typename Profile>
index_type hipified_offloader<Profile>::do_compress(
    ...
    hipified_buffer<value_type> data_buffer{num_elements(data_size)};
    CHECKED_CUDA_CALL(
            hipMemcpy, data_buffer.get(), data, data_buffer.size() * bytes_of<value_type>, hipMemcpyHostToDevice);

    hipified_compressor_impl<Profile> compressor{nullptr /* stream */, data_size};
    hipified_buffer<bits_type> stream_buf{compressed_length_bound<value_type>(data_size)};
    hipified_buffer<index_type> stream_length_buf{1};

    hipified_event start, stop;
    ...
    CHECKED_CUDA_CALL(
            hipMemcpy, &stream_length, stream_length_buf.get(), sizeof stream_length, hipMemcpyDeviceToHost);

    const auto stream_size = stream_length * sizeof(bits_type);
    CHECKED_CUDA_CALL(hipMemcpy, raw_stream, stream_buf.get(), stream_size, hipMemcpyDeviceToHost);

    return stream_length;
}

template<typename Profile>
index_type hipified_offloader<Profile>::do_decompress(const bits_type *raw_stream, index_type length, value_type *data,
    ...
    hipified_buffer<bits_type> stream_buf{length};
    hipified_buffer<value_type> data_buf{num_elements(data_size)};

    CHECKED_CUDA_CALL(hipMemcpy, stream_buf.get(), raw_stream, length * sizeof(bits_type), hipMemcpyHostToDevice);
    ...
    hipified_decompressor_impl<Profile>{nullptr /* stream */}.decompress(stream_buf.get(), data_buf.get(), data_size);
    ...
    CHECKED_CUDA_CALL(hipMemcpy, data, data_buf.get(), data_buf.size() * sizeof(value_type), hipMemcpyDeviceToHost);

    return num_stream_words;
}

extern template class hipified_offloader<profile<float, 1>>;
extern template class hipified_offloader<profile<float, 2>>;
extern template class hipified_offloader<profile<float, 3>>;
extern template class hipified_offloader<profile<double, 1>>;
extern template class hipified_offloader<profile<double, 2>>;
extern template class hipified_offloader<profile<double, 3>>;

#ifdef SPLIT_CONFIGURATION_hipified_encoder
template class hipified_offloader<profile<DATA_TYPE, DIMENSIONS>>;
#endif
