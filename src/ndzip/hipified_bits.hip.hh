...
inline void hip_check(hipError_t err, const char *tag) {
    if (err != hipSuccess) { throw std::runtime_error(std::string{tag} + hipGetErrorString(err)); }
}

#define CHECKED_CUDA_CALL(f, ...) ::ndzip::detail::gpu_hipified::hip_check(f(__VA_ARGS__), STRINGIZE(f) ": ")
...
class hip_buffer {
  public:
    ...
    explicit hip_buffer(index_type size) : _size(size) { CHECKED_CUDA_CALL(hipMalloc, &_memory, size * sizeof(T)); }

    hip_buffer(hip_buffer &&other) noexcept {
        std::swap(_memory, other._memory);
        std::swap(_size, other._size);
    }

    hip_buffer &operator=(hip_buffer &&other) noexcept {
        reset();
        std::swap(_memory, other._memory);
    ...
    void allocate(index_type size) {
        reset();
        CHECKED_CUDA_CALL(hipMalloc, &_memory, size * sizeof(T));
        _size = size;
    }

    void reset() {
        if (_memory) { CHECKED_CUDA_CALL(hipFree, _memory); }
        _memory = nullptr;
        _size = 0;
    }
...
class hip_event {
  public:
    ...
    hip_event() noexcept = default;

    hip_event(allocate_t) { CHECKED_CUDA_CALL(hipEventCreate, &_evt); }
    ...
    void reset() {
        if (_evt) {
            CHECKED_CUDA_CALL(hipEventDestroy, _evt);
            _evt = nullptr;
        }
    }

    void record() {
        if (!_evt) { CHECKED_CUDA_CALL(hipEventCreate, &_evt); }
        CHECKED_CUDA_CALL(hipEventRecord, _evt);
    }

    friend kernel_duration operator-(const hip_event &a, const hip_event &b) {
        assert(a);
        assert(b);
        CHECKED_CUDA_CALL(hipEventSynchronize, a._evt);
        float ms;
        CHECKED_CUDA_CALL(hipEventElapsedTime, &ms, b._evt, a._evt);
        return std::chrono::duration_cast<kernel_duration>(std::chrono::duration<float, std::milli>(ms));
    }

  private:
    hipEvent_t _evt = nullptr;
};
...
auto hierarchical_inclusive_scan_allocate(index_type n_elems) {
    ...
    std::vector<hip_buffer<Scalar>> intermediate_bufs;
    auto n = n_elems;
...
template<typename Scalar, typename BinaryOp>
void hierarchical_inclusive_scan(Scalar *in_out_buf, std::vector<hip_buffer<Scalar>> &intermediate_bufs,
        index_type n_elems, BinaryOp op = {}, hipStream_t stream = nullptr) {
    constexpr index_type granularity = hierarchical_inclusive_scan_granularity;
    constexpr index_type threads_per_block = 256;
